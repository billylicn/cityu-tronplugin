...
export const GRADE_CACHE_KEY = "gradeCacheV1";
...
const GRADE_FIELDS = ["courseId", "courseName", "semesterId", "semesterName", "sourceType", "sourceId", "title", "submitted", "score", "scoreText", "scoreStatus", "scorePublished", "weight", "submittedAt", "directUrl"];
...
export function createGradeCache(value, savedAt = new Date()) {
  ...
  return {
    ...
    terms: list(source.terms).map((term) => ({
      ...
      grades: list(term?.grades).map((item) => pick(item, GRADE_FIELDS)),
