...
test("分享图只绘制匿名汇总与作业缺交率，不绘制需留意明细", () => {
  const source = readFileSync(new URL("../lib/report-image.js", import.meta.url), "utf8");
  assert.match(source, /const WIDTH = 1080/);
  assert.match(source, /const HEIGHT = 1440/);
  assert.match(source, /出勤稳定度/);
  assert.match(source, /作业缺交率/);
  assert.match(source, /项目二维码/);
  assert.doesNotMatch(source, /作业生存率|需要留意的记录|drawRecordColumn|absenceRecords|missingRecords|courseName|shortDate|attendanceText/);
});
